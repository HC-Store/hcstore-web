...
export class PagamentoComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.itensCarrinho = JSON.parse(localStorage.getItem('checkoutData') || '[]');
  }

  get subtotal(): number {
    return this.itensCarrinho.reduce((total, item) => {
      return total + (item.produto?.preco || item.price || 0) * (item.quantidade || item.qty || 1);
    }, 0);
  }
  ...
  concluirCompra(): void {
    ...
    if (!numero.trim() || !validade.trim() || !cvv.trim() || !nome.trim()) {
    ...
    // 🔌 PONTO DE INTEGRAÇÃO COM API DE PAGAMENTO
    // Substitua o setTimeout abaixo pela chamada real da API (ex: Stripe, Mercado Pago)
    // Exemplo Mercado Pago:
    // this.mercadoPagoService.pagar({ numero, validade, cvv, nome, total: this.subtotal }).subscribe(...)
